...
\newlength{\episidewidth}
\setlength{\episidewidth}{0mm}
\newcommand{\episidecolor}{white}

\renewcommand{\epigraph}[3][0.5]{\begin{tcolorbox}[enhanced jigsaw,flush right,title={\lowercase{#3}},halign=right,detach title,fonttitle=\scshape\color{black},after upper={\vspace{0.3em}\par\hfill--- \tcbtitle},colback=white,text width={#1\textwidth},boxrule=0mm,left=-15mm,right=7mm,before skip=20pt,after skip=20pt,top=5mm,borderline east={\episidewidth}{0pt}{\episidecolor}]{\itshape #2}\end{tcolorbox}}

\newcommand{\twinEpigraphs}[4]{%
\begin{tcolorbox}[enhanced jigsaw,sidebyside,sidebyside align=bottom,segmentation style={draw=\episidecolor,solid,line width=\episidewidth},colback=white,text width=\textwidth,boxrule=0mm,left=-5mm,right=0mm,before skip=20pt,after skip=20pt]%
{\flushright{\itshape\small#1}\\\phantom{ }\par\hfill\scshape\color{black}--- \lowercase{#2}}%
\tcblower%
{\flushleft{\itshape\small#3}\\\phantom{ }\par\scshape\color{black}--- \lowercase{#4}}%
\end{tcolorbox}%
}

\newcommand{\mediaepigraph}[4][by]{\epigraph{#2}{``#3'', #1\ #4}}
...
\newcommand{\alice}{%
    ...
    \epigraph{\alice@phrase}{\lowercase{\alice@attr}}
}
\makeatother